...
export class Component {
	...
	async load(jsRuntime: Runtime, filePath: string, type?: 'server' | 'client') {
		let v1CompatModule = { component: null }
		const module = await jsRuntime
			.run(filePath, {
				defineComponent: (x: any) => x,
				console: this.console,
				Bridge: this.v1Compat ? v1Compat(v1CompatModule, this.fileType) : undefined,
			})
			.catch(err => {
	...
	create(fileContent: any, template: any, location = `minecraft:${this.fileType}`, operation?: (deepMerge: (oldData: any, newData: any) => any, oldData: any, newData: any) => any) {
		const keys = location.split('/')
		const lastKey = keys.pop()!

		const current = this.getObjAtLocation(fileContent, [...keys])

		current[lastKey] = (operation ? (oldData: any, newData: any) => operation(deepMerge, oldData, newData) : deepMerge)(current[lastKey] ?? {}, template ?? {})
	}
	...
	async processTemplates(fileContent: any, componentArgs: any, location: string) {
		if (typeof this.template !== 'function') return

		// Try getting file identifier
		const identifier: string = fileContent[`minecraft:${this.fileType}`]?.description?.identifier ?? 'bridge:no_identifier'
		...
		const projectNamespace = this.projectConfig?.get()?.namespace ?? 'bridge'
		...
		const animationController = (animationController: any, molangCondition?: string) => {
			this.animationControllers.push([animationController, molangCondition])
		...
		const recipe = (recipeDef: any) => {
			this.serverFiles.push([`recipes/${folderNamespace}/${this.getShortAnimName('recipe', fileName, this.serverFiles.length)}.json`, recipeDef])
		}
		const spawnRule = (spawnRuleDef: any) => {
			this.serverFiles.push([`spawn_rules/${folderNamespace}/${this.getShortAnimName('sr', fileName, this.serverFiles.length)}.json`, spawnRuleDef])
		}

		const permutationEventName = (await hashString(`${this.name}/${location}`)).slice(0, 16)
		const onActivated = (eventResponse: any) => this.registerLifecycleHook(fileContent, location, eventResponse, permutationEventName, 'activated')
		const onDeactivated = (eventResponse: any) => this.registerLifecycleHook(fileContent, location, eventResponse, permutationEventName, 'deactivated')
		...
		if (this.fileType === 'entity') {
			this.template(componentArgs ?? {}, {
				...
				create: (template: any, location?: string, operation?: any) => this.create(fileContent, template, location, operation),
				location,
				identifier,
				projectNamespace,
				animationController,
				...
				spawnRule,
				dialogueScene:
					!this.targetVersion || compare(this.targetVersion, '1.17.10', '>=')
						? (scene: any, openDialogue = true) => {
								if (!this.dialogueScenes[fileName]) this.dialogueScenes[fileName] = []
								this.dialogueScenes[fileName].push(scene)

								if (scene.scene_tag && openDialogue)
									onActivated({
										run_command: {
											command: [`/dialogue open @s @p ${scene.scene_tag}`],
										},
									})
						  }
						: undefined,
			...
			this.template(componentArgs ?? {}, {
				...
				player: {
					...
					create: (template: any, location?: string, operation?: any) => {
						this.createOnPlayer.push([location ?? `minecraft:entity`, template, operation])
					},
				},
			})
	...
	async processAdditionalFiles(filePath: string, fileContent: any, isPlayerFile = false) {
		const bpRoot = this.projectConfig?.getRelativePackRoot('behaviorPack') ?? 'BP'
		const rpRoot = this.projectConfig?.getRelativePackRoot('resourcePack')
		// Try getting file identifier
		const identifier = isPlayerFile ? 'minecraft:player' : fileContent[`minecraft:${this.fileType}`]?.description?.identifier ?? 'bridge:no_identifier'
		...
		return {
			...
			[join(bpRoot, `dialogue/${folderNamespace}/${fileName}.json`)]:
				this.dialogueScenes[fileName] && this.dialogueScenes[fileName].length > 0
					? {
							baseFile: filePath,
			...
			...Object.fromEntries(
				Object.entries(this.clientFiles).map(([currFilePath, jsonContent]) => [
		...
		for (const [anim, condition] of this.animations) {
			...
			if (condition !== false)
				...
				this.create(
					...
					{
						scripts: {
							animate: [!condition ? shortAnimId : { [shortAnimId]: condition }],
						},
					},
					'minecraft:entity/description'
				)
	 ...
	 */
	protected registerLifecycleHook(fileContent: any, location: string, eventResponse: any, permutationEventName: string, type: 'activated' | 'deactivated') {
		if (!fileContent[`minecraft:${this.fileType}`].events) fileContent[`minecraft:${this.fileType}`].events = {}
		...
		if (type === 'activated' && location === `minecraft:${this.fileType}/components`) {
			if (!entityEvents['minecraft:entity_spawned']) entityEvents['minecraft:entity_spawned'] = {}
			...
			const keys = location.split('/')
			if (keys.pop() !== 'components') throw new Error('Invalid component location inside of permutation')
	 ...
	 */
	protected findComponentGroupReferences(events: any, type: 'add' | 'remove', componentGroupName: string) {
		let eventsWithComponentGroups: any[] = []

		for (const eventName in events) {
			const event = events[eventName]

			if (Array.isArray(event.sequence)) eventsWithComponentGroups.push(...this.findComponentGroupReferences(event.sequence, type, componentGroupName))
			else if (Array.isArray(event.randomize)) eventsWithComponentGroups.push(...this.findComponentGroupReferences(event.randomize, type, componentGroupName))
			else {
				const componentGroups = event[type]?.component_groups ?? []
				if (componentGroups.includes(componentGroupName)) eventsWithComponentGroups.push(event)
			}
		}
