...
export class Component {
	...
	create(
		...
		operation?: (deepMerge: (oldData: any, newData: any) => any, oldData: any, newData: any) => any
	) {
		...
		current[lastKey] = (
			operation ? (oldData: any, newData: any) => operation(deepMerge, oldData, newData) : deepMerge
	...
	async processTemplates(fileContent: any, componentArgs: any, location: string) {
		...
		const identifier: string =
			fileContent[`minecraft:${this.fileType}`]?.description?.identifier ?? 'bridge:no_identifier'
		...
		const onActivated = (eventResponse: any) =>
			this.registerLifecycleHook(fileContent, location, eventResponse, permutationEventName, 'activated')
		const onDeactivated = (eventResponse: any) =>
			this.registerLifecycleHook(fileContent, location, eventResponse, permutationEventName, 'deactivated')
	...
	protected registerLifecycleHook(
		...
		if (type === 'activated' && location === `minecraft:${this.fileType}/components`) {
			...
			const eventName = `bridge:${permutationEventName}_${type}_${
				type === 'activated' ? this.lifecycleHookCount.activated++ : this.lifecycleHookCount.deactivated++
			}`
		...
		for (const eventName in events) {
			...
			if (Array.isArray(event.sequence))
				eventsWithComponentGroups.push(
					...this.findComponentGroupReferences(event.sequence, type, componentGroupName)
				)
			else if (Array.isArray(event.randomize))
				eventsWithComponentGroups.push(
					...this.findComponentGroupReferences(event.randomize, type, componentGroupName)
				)
			else {
