...
export function createCustomComponentPlugin({ fileType, getComponentObjects }: IOpts): TCompilerPluginFactory<{
	v1CompatMode?: boolean
}> {
	const usedComponents = new Map<string, [string, string][]>()
	let createAdditionalFiles: Record<string, { baseFile: string; fileContent: any }> = {}

	return ({ console, projectConfig, projectRoot, compileFiles, getAliases, getAliasesWhere, options, jsRuntime, targetVersion, fileType: fileTypeLib, fileSystem }) => {
		let playerFile: string | null = null
		const isPlayerFile = (filePath: string | null, getAliases: (file: string) => string[]) => {
			if (!filePath) return false
			if (playerFile && filePath === playerFile) return true

			const isPlayerFile = fileType === 'item' && fileTypeLib?.getId(filePath) === 'entity' && getAliases(filePath).includes('minecraft:player')
		...
		const isComponent = (filePath: string | null) => {
			...
			if (cachedIsComponent.has(filePath)) return cachedIsComponent.get(filePath)!

			const isComponent = options.v1CompatMode ? filePath.includes(`/components/`) : fileTypeLib?.getId(filePath) === `customComponent` && filePath.includes(`/${fileType}/`)
		...
		const mayUseComponent = (filePath: string | null) => {
			...
			if (cachedMayUseComponents.has(filePath)) return cachedMayUseComponents.get(filePath)
		...
		return {
			...
			transformPath(filePath) {
				if (isComponent(filePath) && options.buildType !== 'fileRequest') return null
			},
			async read(filePath, fileHandle) {
				...
				if (!fileHandle) return createAdditionalFiles[filePath] ? json5.parse(createAdditionalFiles[filePath].fileContent) : undefined
				...
				if (isComponent(filePath)) {
					...
					try {
						...
						if (options.buildType !== 'fileRequest') console.error(`Error within file "${filePath}": ${err}`)
						return {
			...
			async load(filePath, fileContent) {
				...
				if (isComponent(filePath) && typeof fileContent === 'string') {
					const component = new Component(console, fileType, fileContent, options.mode, !!options.v1CompatMode, targetVersion)
			...
			async transform(filePath, fileContent, dependencies = {}) {
				...
				if (isPlayerFile(filePath, getAliases)) {
					...
					const itemComponents = <Component[]>Object.entries(dependencies)
						.filter(([depName]) => depName.startsWith('itemComponent#'))
					...
					// Apply components
					for (const [componentName, location] of usedComponents.get(filePath) ?? []) {
						const component = <Component>dependencies[`${fileType}Component#${componentName}`]
			...
			finalizeBuild(filePath, fileContent) {
				...
				} else if (mayUseComponent(filePath) || createAdditionalFiles[filePath]) return JSON.stringify(fileContent, null, '\t')
			},
			async buildEnd() {
				if (!hasComponentFiles) return
				...
				createAdditionalFiles = Object.fromEntries(
					Object.entries(createAdditionalFiles)
						.filter(([_, fileData]) => fileData?.fileContent !== undefined)
						.map(([filePath, fileData]) => [join(projectRoot, filePath), fileData])
				)
...
export const CustomEntityComponentPlugin = createCustomComponentPlugin({
	...
	getComponentObjects: fileContent => [
		['minecraft:entity/components', fileContent?.['minecraft:entity']?.components ?? {}],
		...Object.entries(fileContent?.['minecraft:entity']?.component_groups ?? {}).map(
			([groupName, groupContent]) => <[string, any]>[`minecraft:entity/component_groups/${groupName}`, groupContent]
		),
		...(<any[]>fileContent?.['minecraft:entity']?.permutations ?? []).map(
			(permutation: any, index: number) => <[string, any]>[`minecraft:entity/permutations/${index}/components`, permutation?.components ?? {}]
		),
	],
})
...
export const CustomItemComponentPlugin = createCustomComponentPlugin({
	fileType: 'item',
	getComponentObjects: fileContent => [['minecraft:item/components', fileContent?.['minecraft:item']?.components ?? {}]],
})

export const CustomBlockComponentPlugin = createCustomComponentPlugin({
	fileType: 'block',
	getComponentObjects: fileContent => [
		['minecraft:block/components', fileContent?.['minecraft:block']?.components ?? {}],
		...(<any[]>fileContent?.['minecraft:block']?.permutations ?? []).map(
			(permutation, index) => <[string, any]>[`minecraft:block/permutations/${index}/components`, permutation.components ?? {}]
		),
	],
})
